...
</div>`;M&&M.forEach(e=>{e.addEventListener("click",()=>{openRightPanel(),A.innerHTML=te,H()})});S&&S.forEach(e=>{e.addEventListener("click",()=>{openRightPanel(),A.innerHTML=re,H()})});const $=F("id");let _="";document.addEventListener("DOMContentLoaded",()=>{document.querySelector("#share").addEventListener("click",ie),document.querySelectorAll(".reserving").forEach(e=>e.addEventListener("click",oe))});z($).then(e=>{{(!e||!e.brand)&&confirm("Данный автомобиль снят с продажи")&&window.history.back(),_=document.querySelectorAll(".auto-name"),_[0].innerHTML=_[1].innerHTML=_[2].innerHTML=e.brand+" "+e.model+", "+e.yearReleased;let t=document.querySelector("#auto-brand-path");t.innerHTML=`<a href='/cars/?brand=${e.brand}&brandId=${e.brandId||""}'>Автомобили ${e.brand} с пробегом</a>`;let i=document.querySelectorAll(".auto-price");i[0].innerHTML=i[1].innerHTML=n(e.price)+" ₽";let c=document.querySelectorAll(".auto-price-month");c[0].innerHTML=c[1].innerHTML="от "+parseInt(parseInt(e.price.replace(/ /g,""))/90.12)+" ₽/мес";let d=document.querySelectorAll(".auto-year");d[0].innerHTML=d[1].innerHTML=d[1].innerHTML=e.yearReleased+" год";let p=document.querySelectorAll(".auto-mill");p[0].innerHTML=p[1].innerHTML=n(e.milleage)+" км";let f=document.querySelectorAll(".auto-engine");f[0].innerHTML=f[1].innerHTML=e.engineCapacity+" л / "+e.enginePower+" л.с / "+e.engineType;let v=document.querySelectorAll(".auto-gear");v[0].innerHTML=v[1].innerHTML=e.gearboxType||"";let a=document.querySelectorAll(".auto-drive");a[0].innerHTML=a[1].innerHTML=e.driveType||"";let g=document.querySelectorAll(".auto-rule");g[0].innerHTML=g[1].innerHTML=e.wheelType||"";let r=document.querySelectorAll(".auto-body");r[0].innerHTML=r[1].innerHTML=e.bodyType||"";let o=document.querySelectorAll(".auto-color");o[0].innerHTML=o[1].innerHTML=e.color||"";let x=document.querySelector(".main__card-product--top .address");x.innerHTML=e.fullAddress}{let t=document.querySelector("#auto-swip");e.images.forEach(c=>{t.innerHTML+=`
    <div class="swiper-slide">
        <img src="${c}" alt="">
    </div>`}),setTimeout(()=>P(),3e3);let i=document.querySelector("#auto-more");i.innerHTML=`<div class="fotos_black" onclick="showMore()">${e.images.length} фото</div>`,e.images.forEach((c,d)=>{i.innerHTML+=`<div onClick="showSlide(${d})"><img src="${c}" alt=""></div>`})}{let t=document.querySelector("#auto-equipment");t.innerHTML=[],e.сonfiguration.forEach(i=>t.innerHTML+=`<li>${i}</li>`)}{let t=document.querySelector("#favorite_chosen");t&&(t.innerHTML=`<a href="javascript:addCompare('${e.id}')" ondblclick="dblCompare('${e.id}')">
