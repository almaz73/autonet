...
<div class="request_vac">
    ...
    </div>`}),document.querySelector("#vacancyPlace").innerHTML=t,f(!0)}function f(n){document.querySelectorAll(".vacancy_item").forEach(t=>{const e=t.querySelector(".fst_desc").offsetHeight;t.style.maxHeight=e+"px",n?t.querySelector(".fst_desc").addEventListener("click",s):t.querySelector(".fst_desc").removeEventListener("click",s);function s(){t.classList.contains("expanded")?(t.classList.remove("expanded"),t.style.maxHeight=e+"px"):(t.classList.add("expanded"),t.style.maxHeight="inherit")}})}function x(){u(),document.querySelectorAll(".vacancy_item").forEach(n=>{n.classList.remove("expanded");const l=n.querySelector(".fst_desc").offsetHeight;n.style.maxHeight=l+"px"})}window.addEventListener("DOMContentLoaded",()=>{_(),u(),window.addEventListener("resize",x);const n=document.querySelector('.generalForm [name="resume"]'),t=document.querySelector("#fN");n.addEventListener("change",function(e){this.files&&this.files[0]?t.textContent=this.files[0].name:t.textContent="Загрузить файл"});function l(e,s,a){let i=!1;return e.value?e.style.border="":(e.style.border="1px solid red",r("Не заполнено поле ФИО","warning"),i=!0),s.value?s.style.border="":(s.style.border="1px solid red",r("Не заполнен Телефон","warning"),i=!0),a.checked?a.parentNode.style.border="":(a.parentNode.style.border="1px solid red",r("Вы должны дать согласие на обработку персональных данных","warning"),i=!0),i}window.sendResume=function(e){const s=document.querySelector(`.${e} [name="fio"]`),a=document.querySelector(`.${e} [name="city"]`),i=document.querySelector(`.${e} [name="phone"]`),o=document.querySelector(`.${e} [name="email"]`),c=document.querySelector(`.${e} [name="text"]`),m=document.querySelector(' [name="resume"]'),v=document.querySelector(`.${e} [type="checkbox"]`);if(e==="generalForm"){const d=document.querySelector(".modal__error");d.style.display=v.checked?"none":"block"}if(l(s,i,v))return!1;let y={fullName:s.value,phone:i.value,email:o&&o.value,city:a&&a.value,aboutYourself:c&&c.value,resume:m&&m.files[0]};h(y).then(d=>{d&&d.ok?r("Ваше сообщение успешно получено!"):r("Сервер не принял","error")})}});
