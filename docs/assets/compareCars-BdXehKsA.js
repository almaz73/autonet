function n(r){return parseInt(r).toLocaleString("ru-RU")}function S(r){let e=[];return r&&r.forEach(t=>{let d=n(t.milleage)+" км, ";t.engineCapacity&&(d+=t.engineCapacity),t.gearboxType&&(d+=" "+t.gearboxType),t.enginePower&&(d+=" ("+t.enginePower+"&nbsp;л.с)"),t.bodyType&&(d+=", "+t.bodyType),d+=", "+t.driveType,t.engineType&&(d+=", "+t.engineType);let i=n(parseInt(parseInt(t.price.replace(/ /g,""))/90.12));e.push({address:t.fullAddress,id:t.id,name:t.brand+" "+t.model,href:"/cars/car.html?id="+t.id,price:n(t.price),fromPerMonth:i,info:d,photos:t.images})}),e}window.addCompare=function(r){let e=window.compareCars.find(s=>s.id===r),t=l(),d=e;d.images=e.images[0];let i=document.querySelector("#compareId_"+r),o=i.classList.contains("chosen"),c=t.find(s=>s.id===r);o?(i.classList.remove("chosen"),t=t.filter(s=>s.id!==d.id)):(i.classList.add("chosen"),c||t.push(d)),localStorage.setItem("ComparedCars",JSON.stringify(t)),p(t)};function l(){let r=localStorage.getItem("ComparedCars");return r?r=JSON.parse(r):r=[],r}function p(r){let e=document.querySelector("#compareCount");e&&r.length?(e.innerHTML='<img src="/icons/car-icon_b.svg">'+r.length,e.style.display="flex"):e&&(e.style.display="none")}window.deleteAllCar=function(){window.deleteCar()};window.deleteCar=function(r){let e=l();if(r?e=e.filter(t=>t.id!==r):e=[],localStorage.setItem("ComparedCars",JSON.stringify(e)),p(e),P(e),!e.length){let t=document.querySelector(".compare-clear");t&&(t.style.display="none")}};function P(r){let e=r||l();if(p(e),e.forEach(t=>{let d=document.querySelector("#compareId_"+t.id);d&&d.classList.add("chosen")}),location.pathname==="/personal/list-compared/"){let t=document.querySelector("#compare-field"),d="",i="",o="",c="",s="",f="",m="",u="",g="",T="",y="",E="",$="",h="",C="",I="";e.forEach(a=>{d+=`<td><a href="javascript:deleteCar('${a.id}')">Удалить</a></td>`,i+=`<td><a href="${a.href}"><img src="${a.images}" alt=""></a></td>`,o+=`<td><a href="/cars/car.html?id=${a.id}">"${a.brand} ${a.model}</a></td>`,c+=`<td>${n(a.price)} руб.</td>`,a.milleage&&(s+=`<td>${n(a.milleage)||""} км</td>`),f+=`<td>${a.yearReleased||""}</td>`,m+=`<td>${a.color||""}</td>`,u+=`<td>${a.engineCapacity||""}</td>`,g+=`<td>${a.engineType||""}</td>`,T+=`<td>${a.enginePower||""}</td>`,y+=`<td>${a.gearboxType||""}</td>`,E+=`<td>${a.driveType||""}</td>`,$+=`<td>${a.bodyType||""}</td>`,h+=`<td>${a.fullAddress||""}</td>`,C+=`<td>${a.brand||""}</td>`,I+=`<td>${a.model||""}</td>`}),t.innerHTML=`
              <tr class="DELETE">
